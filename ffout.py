...
def perform_calculations():
    ...
    #calculate deductions
        ...
        socsec = pay * DED_RATE[2]
        medi = pay * DED_RATE[3]
        ret401 = pay * DED_RATE[4]
    #STUDENTS: calculate other deductions here:

        net = pay - fed - state - socsec - medi - ret401
    ...
    #append amounts to lists
        ...
        soc_sec.append(socsec)
        medicare.append(medi)
        ret401k.append(ret401)
        net_pay.append(net)
...
def create_output_file():
    ...
    for i in range(num_emps):
        ...
        data_fed_tax = format(fed_tax[i], currency)
        data_state_tax = format(state_tax[i], currency)
        data_soc_sec = format(soc_sec[i], currency)
        data_medicare = format(medicare[i], currency)
        data_net_pay = format(net_pay[i], currency)
        
        f.write(data1 + tab + data_fed_tax + tab + data_state_tax + tab + data_soc_sec + tab + data_medicare + tab + data_net_pay)
